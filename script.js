...
function saveCoinsToSystem(finalScore) {
    ...
    if (url.includes("СЮДА_ТЫ_ВСТАВИШЬ_ССЫЛКУ")) {
        localStorage.setItem("total_caps", parseInt(localStorage.getItem("total_caps") || "0") + finalScore);
        return; 
    }
...
function Dinosaur (x, dividerY) {
    ...
    this.vy = 0; this.animTicks = 0; this.bobY = 0; this.landTimer = 0; this.isLanding = false;
}
Dinosaur.prototype.draw = function(ctx) {
    let img = this.isLanding ? imgLand : (this.y < this.baseY ? imgJump : imgRun);
    ctx.drawImage(img, this.x, this.y + this.bobY, this.width, this.height);
};
Dinosaur.prototype.update = function(mod) { this.animTicks += mod * 60; this.bobY = Math.sin(this.animTicks * 0.1) * 3; if(this.isLanding) { this.landTimer -= mod * 60; if(this.landTimer <= 0) this.isLanding = false; } };
Dinosaur.prototype.jump = function(type) { if (this.y >= this.baseY) this.vy = (type === 'long') ? -15.5 : -12; };
Dinosaur.prototype.physics = function(grav, mod) {
    this.y += this.vy * (mod * 60); this.vy += grav * (mod * 60);
...
PostApocalypseSun.prototype.draw = function(ctx) {
    ctx.fillStyle = "#FF4500"; ctx.beginPath(); ctx.arc(550, 60, 20, 0, Math.PI * 2); ctx.fill();
};
...
CityBackground.prototype.update = function(speed, mod) { this.buildings.forEach(b => { b.x += speed * 0.15 * (mod * 60); if(b.x + b.w < 0) b.x = 640; }); };
...
// --- ГЛАВНЫЙ КЛАСС ИГРЫ (ОСТАЛЬНАЯ ЛОГИКА ТВОЯ) ---
function Game() {
    ...
    this.canvas.addEventListener('touchstart', () => { this.dino.jump('normal'); }, { passive: false });
}

Game.prototype.update = function(mod) {
    if(this.paused) return;
    this.dino.update(mod); this.dino.physics(0.6, mod);
    this.cityBg.update(this.runSpeed, mod);
    this.score += 0.1;
};
...
var game = new Game();
function main(now) {
    let mod = (now - game.lastTime) / 1000;
    game.lastTime = now;
    game.update(mod);
    game.draw(mod);
